...
const handleImportClick = async () => {
  ...
  try {
    ...
    // 用 updatedUserData 同步本地 formData.Abyss。
    const updated = response?.updatedUserData
    if (updated?.Abyss) {
      props.formData.Abyss = {
        Snapshots: (updated.Abyss as HSRUserConfigAbyss).Snapshots ?? '{}',
      }
    }
    // 摘要消息
    const successCount = Array.isArray(response?.items)
      ? response.items.filter((i: any) => i.success).length
      : 0
    message.success(
      response?.message || `已从 M7A config.yaml 导入 ${successCount}/3 个三深渊快照`,
    )
...
const collectTeams = (snapshot: Record<string, any>, prefix: string) => {
...
const parseSnapshotCollection = (): Partial<Record<HSRAbyssKey, Record<string, any>>> => {
