  ...
  <div class="form-section">
    ...
    <a-row :gutter="24" style="margin-top: 8px">
      ...
      <a-col :span="12">
        <a-form-item>
          <template #label>
            <a-tooltip title="到达开始日且本周未完成时，MAS 会交给 M7A/SRA 尝试完成历战余响；日志确认完成后本周不再执行。">
              <span class="form-label">
                历战余响开始日
                <QuestionCircleOutlined class="help-icon" />
              </span>
...
import type { HSRUserConfig_Stage, HSRUserConfig_TaskOpt } from '@/api'
import type {
...
// HSR 专用：Stage Section Props
// formData 由父级传入完整 HSRUserConfigData（包含 Stage / Info / TaskSwitch / TaskOpt），
// 这里只直接处理 Stage / TaskOpt。loading 用于保存中禁用下拉，避免重复点击被父级 isSaving guard 吞掉。
const props = defineProps<{
  formData: { Stage: HSRUserConfig_Stage; TaskOpt: HSRUserConfig_TaskOpt; [k: string]: any }
  loading: boolean
...
const scriptStageContainer = computed<HSRScriptStageContainer | null>(() => {
  const payload = parseScriptStage(props.formData.Stage.ScriptStage) as HSRScriptStageContainer | null
...
const nativeEngineMismatch = computed(() => {
  ...
  return (!!main?.engine && main.engine !== props.dailyEngine)
...
const saveNativeMainStage = (
  channel: ActiveChannel,
  option: HSRDynamicStageOption | null
) => {
  const container = scriptStageContainer.value
  ...
  props.formData.Stage.ScriptStage = value
  emitSave('Stage.ScriptStage', value)
}

const saveNativeEchoOfWarStage = (option: HSRDynamicStageOption | null) => {
  const value = option
    ? JSON.stringify(buildNativeStagePayload(option))
    : emptyNativeStageValue
  props.formData.Stage.ScriptEchoOfWar = value
...
const handleActiveChannelChange = (value: ActiveChannel) => {
  ...
  props.formData.Stage.Channel = value
